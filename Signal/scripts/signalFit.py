...
nominalWSFileName = glob.glob("%s/output_%s_M%s.root"%(opt.inputWSDir,opt.proc,MHNominal))[0]
...
if opt.skipZeroes:
  ...
  WSFileName = glob.glob("%s/output_%s_M%s.root"%(opt.inputWSDir,opt.proc,MHNominal))[0]
...
for mp in opt.massPoints.split(","):
  ...
  WSFileName = glob.glob("%s/output_%s_M%s.root"%(opt.inputWSDir,procRVFit,mp))[0]
  f = ROOT.TFile(WSFileName,"read")
  inputWS = f.Get(inputWSName__)
...
if( datasetRVForFit[MHNominal].numEntries() < opt.replacementThreshold  )|( datasetRVForFit[MHNominal].sumEntries() < 0. ):
  ...
  for mp in opt.massPoints.split(","):
    ...
    WSFileName = glob.glob("%s/output_%s_M%s.root"%(opt.inputWSDir,procReplacementFit,mp))[0]
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
...
if not opt.skipVertexScenarioSplit:
  ...
  for mp in opt.massPoints.split(","):
    ...
    WSFileName = glob.glob("%s/output_%s_M%s.root"%(opt.inputWSDir,procWVFit,mp))[0]
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
...
ssfRV = SimultaneousFit(name,opt.proc,opt.cat,datasetRVForFit,xvar.Clone(),MH,MHLow,MHHigh,opt.massPoints,opt.nBins,opt.MHPolyOrder,opt.minimizerMethod,opt.minimizerTolerance,opt.doVoigtian,opt.freezeGammaH) #JTao
if opt.useDCB:
   if opt.doVoigtian:
      print "******JTao: constructing DCB+Voigtian for RV*****"
      ssfRV.buildDCBplusVoigtian() #JTao
   else:
      print "******JTao: constructing DCB+Gaussian for RV*****"
      ssfRV.buildDCBplusGaussian()
else: 
  if opt.doVoigtian:
     print "******JTao: constructing NVoigtians for RV*****"
     ssfRV.buildNVoigtians(nRV) #JTao
  else:
     print "******JTao: constructing NGaussians for RV*****"
...
if not opt.skipVertexScenarioSplit:
  name = "WV"
  ssfWV = SimultaneousFit(name,opt.proc,opt.cat,datasetWVForFit,xvar.Clone(),MH,MHLow,MHHigh,opt.massPoints,opt.nBins,opt.MHPolyOrder,opt.minimizerMethod,opt.minimizerTolerance,opt.doVoigtian,opt.freezeGammaH)
  if opt.useDCB:
     if opt.doVoigtian:
        print "******JTao: constructing DCB+Voigtian for WV*****"
        ssfWV.buildDCBplusVoigtian() #JTao
     else:
        print "******JTao: constructing DCB+Gaussian for WV*****"
        ssfWV.buildDCBplusGaussian()
  else:
    if opt.doVoigtian:
       print "******JTao: constructing NVoigtians for WV*****"
       ssfWV.buildNVoigtians(nWV) #JTao
    else:
       print "******JTao: constructing NGaussians for WV*****"
       ssfWV.buildNGaussians(nWV)
  ssfWV.runFit()
  ssfWV.buildSplines()
  ssfMap[name] = ssfWV
